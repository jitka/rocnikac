\chapter{Application on clique game}
...
\sec{weak pn-search} \ref{weak} too which depends on branching factor and it decreases.
...
considering changes in \fce{updateAncestor()} when node has more then one parent.
...
tries to find it. \obr{ 10) obrazek obteceni}. 
...
when we use also \sec{deleting solved subtrees} \label{delete} ---
...
bits. Supposing that size of \TODO{tomovi se nelibi int} is at least ${N
...
\& \; v \ne j$). If we use \sec{Adjacency matrix in line}, there is a~trick. We can
...
(j\!+\!1)\!*\!N\!-\!1 $. Then we just \fce{and} these rows. Every nonzero bit
indicates such $v$.
...
of four nodes which contain $i$ and $j$. \TODO{ \obr{13z)} }
...
$(i,v)$. If yes then $i$,$j$,$u$ and $v$ is threat.  \obr{ 13a) hrozba}

Second case is similar to finding claim winning set. We just check if there is free
edge between $(u,v)$ instead of red edge.  \obr{ 13b) hrozba}
...
instead of $G$. The enhancement \fce{transpositions into DAG} \ref{DAG} causes that
each subtree of two isomorph graphs will be searched at most once.

\TODO{tomovi nelibi -- The difficulty of this lies} in computing representative
graph, which is hard problem.  \TODO{np-těžký nebo np-úplný? jestě těžší..
citace compiual?  complexity, handbook of complexity} So we will compute almost
...
\subsubsection{Sorting by degree}
